...
class TestDatabaseEndpoints:
    ...
    def test_get_data_time_range(self, client, sample_metrics_data):
        ...
        response = client.get("/api/v1/vms/test-vm-01/metrics/cpu.usage.average/time-range")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_data_completeness(self, client, sample_metrics_data):
        ...
        response = client.get(
            f"/api/v1/vms/test-vm-01/metrics/cpu.usage.average/completeness",
            params={
    ...
    def test_get_missing_data(self, client, sample_metrics_data):
        ...
        response = client.get(
            f"/api/v1/vms/test-vm-01/metrics/cpu.usage.average/missing-data",
            params={
                "start_date": start_date.isoformat(),
                "end_date": end_date.isoformat(),
                "expected_interval_minutes": 30
...
class TestFactsEndpoints:
    ...
    def test_get_metrics_fact(self, client, sample_metrics_data):
        ...
        data = response.json()
        assert len(data) == 10
        assert all(m["vm"] == "test-vm-01" for m in data)
